...
{
    public Transform inside; // middle of home
    public Transform outside; // outside, way to exit
    ...
    private void OnDisable()
    {
        ...
        if (this.gameObject.activeSelf) {
            // coroutine stops execution temporarily for it to stop
    ...
    private IEnumerator ExitTransition()
    {
        ...
        this.ghost.movement.SetDirection(Vector2.up, true); // force it up
        this.ghost.movement.rigidbody.isKinematic = true; // turn off rigidbody
        this.ghost.movement.enabled = false;

        Vector3 position = this.transform.position; // current position

        float duration = 0.5f; // half second to go between each node, 1s for full transition
        float elapsed = 0.0f;
